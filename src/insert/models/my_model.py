...
class InsertAnything(L.LightningModule):
    def __init__(
        ...
        self.lora_layers = self.init_lora(resume_ckpt_path, pretrained_lora_path, lora_config)
        self.to(device).to(dtype)

    def init_lora(self, resume_ckpt_path: str, pretrained_lora_path: str, lora_config: dict):
        # Case 1: Resuming from an interrupted training run (expects a directory)
        if resume_ckpt_path and os.path.isdir(resume_ckpt_path):
            print(f"Resuming LoRA weights from checkpoint directory: {resume_ckpt_path}")
            # This implicitly loads into the 'default' adapter
            self.flux_fill_pipe.load_lora_weights(resume_ckpt_path, adapter_name="default")

        # Case 2: Fine-tuning from a specific pretrained LoRA file
        elif pretrained_lora_path and os.path.isfile(pretrained_lora_path):
            print(f"Initializing new LoRA adapter for fine-tuning...")
            # 1. Add the adapter config first. This creates the 'default' entry in peft_config.
            # self.transformer.add_adapter(LoraConfig(**lora_config), adapter_name="default")
            
            print(f"Loading pretrained LoRA weights into 'default' adapter from: {pretrained_lora_path}")
            # 2. Now load the pretrained weights into the adapter we just registered.
            self.flux_fill_pipe.load_lora_weights(
                os.path.dirname(pretrained_lora_path),
                weight_name=os.path.basename(pretrained_lora_path),
                adapter_name="default" # Explicitly load into the 'default' adapter
            )

        # Case 3: Training from scratch with a new LoRA config
        elif lora_config:
            print("No checkpoint found. Initializing new LoRA layers for training from scratch.")
            # This creates the 'default' adapter
            self.transformer.add_adapter(LoraConfig(**lora_config), adapter_name="default")
            
        else:
            raise ValueError("Must provide one of: resume_ckpt_path, pretrained_lora_path, or lora_config.")

        # This part remains the same, collecting trainable parameters
        lora_layers = filter(
